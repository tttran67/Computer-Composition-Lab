...
module multiply(              // 乘法器
    ...
    begin
        ...
        begin    // 如果正在进行乘法，则被乘数每时钟左移两位
            multiplicand <= {multiplicand[61:0],2'b00};//区别
        end
        else if (mult_begin) 
    ...
    begin
        ...
        begin   // 如果正在进行乘法，则乘数每时钟右移两位
            multiplier <= {2'b00,multiplier[31:2]}; //区别
        end
        else if (mult_begin)
    ...
    wire [63:0] partial_product2;//区别
    assign partial_product1 = multiplier[0] ? multiplicand : 64'd0;
    assign partial_product2 = multiplier[1]?{multiplicand[62:0],1'b0} : 64'd0;//区别
